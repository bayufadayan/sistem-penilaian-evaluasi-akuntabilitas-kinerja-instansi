...
import AdminDeleteButton from "@/app/(admin)/components/buttons/adminDeleteButton";
...
export default function DeleteSubComponent({
  subComponent,
}: {
  ...
  const handleDelete = async (subComponentId: number) => {
    setIsLoading(true);
    await axios.delete(`/api/subcomponents/${subComponentId}`);
    router.refresh();
    setIsLoading(false);
    ...
    <>
      ...
      {isModalOpen && (
        <div
          id="popup-modal"
          tabIndex={-1}
          className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-50 h-full w-full"
        >
          <div className="relative p-4 w-full max-w-md bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div className="p-4 md:p-5 text-center">
              ...
              <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                Apakah Anda yakin ingin menghapus subComponent{" "}
              ...
              {!isLoading ? (
                <button
                  ...
                  onClick={() => handleDelete(subComponent.id)}
                >
                  Ya, Saya yakin
                </button>
              ) : (
