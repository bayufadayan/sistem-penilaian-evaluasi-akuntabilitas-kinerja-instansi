...
type ExcelRow = {
    No: number;
    "Nama Kriteria": string;
    Deskripsi?: string;
};

export default function UploadExcel({ subComponentId }: { subComponentId: string }) {
    ...
    const sendExcelDataToAPI = async (data: ExcelRow[]) => {
        for (const row of data) {
            ...
            const payload = {
                criteria_number: row.No,
                name: row["Nama Kriteria"],
                description: row.Deskripsi || "",
                id_subcomponents: Number.parseInt(subComponentId),
            };

            await axios.post("/api/criterias", payload);
        }
